...
- **The date on the rates is the feed's newest `arrival_date`, not the calendar date**, so on a morning before mandis report, or a day data.gov.in is down, it is yesterday or older. The web headings say "Today's mandi rates" only when that date is today in IST, and "Latest mandi rates · reported 25 Sep" otherwise (`client/src/utils/ratesDate.ts`). The app's rates screen title and the storefront rail follow the same rule (`mobile/src/lib/ratesDate.ts`), and read "Mandi rates" until the date is known.
