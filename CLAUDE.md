...
**`mobile/` is trade-only (done 2026-09-10).** Once Daily carried the shopping surfaces, they came out of the business app rather than being maintained twice:

- **Gone:** `CartContext`, `CartBar`, `QuantityStepper`, `BillDetails`, `cartLines`, `ConsumerTabBar`, `screens/consumer/`, and the pack-pricing half of `lib/catalog.ts` (`shopPack`, `orderQuantity`, `PACKS`, `RETAIL_MARGIN`).
- **`StorefrontHomeScreen` has one mode.** It used to run a second, city-gated retail shelf priced by the household pack with a basket riding the bottom, chosen by a `shopping` flag that was true for CONSUMERS **and guests**. A lot moves by the tonne and can be freighted, so there is no locality to gate on: the market is national and the card action is `BID`, never `ADD`. `trustScore` is rounded here too now, so no card shows `★ 84.2601595017465`.
- **`ListingDetailScreen` lost its retail half**: no household price, no pack stepper, no add-to-basket. A guest gets a sign-in bar, an approved buyer gets the bid form.
- **A signed-in CONSUMER is a prospective partner, not a shopper.** They get `screens/partner/JoinScreen`: pick selling or bulk buying, then the application. Signup no longer offers a CONSUMER pill.

**`OnboardingScreen` now takes the chosen `kind` instead of reading `user.role`.** This was the same bug §4 records the web fixing, still live on mobile: a CONSUMER is by definition somebody with no partner role, so `role === 'FARMER'` was false for **every first-time applicant** and all of them were handed the buyer form whichever card they tapped. A resubmitting FARMER still falls through to their own form via the role check.
