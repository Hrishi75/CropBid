...
**Knowingly unbuilt:** an admin cannot mark a shop `verified` or upload product images from the panel.
...
- **One row per crop per state, updated in place, written once a day**: about 1,600 rows (perhaps 3,000 once a year of seasonal crops has passed) and under 1 MB for good, and the free-plan Neon database is woken once a day for it rather than on every two-hourly refresh. The user asked about the size before agreeing: 0.5 GB is the free limit, CropBid uses about 10 MB, and the "LIMIT 50" in Neon's table viewer is its paging, not a limit on rows.
- **Nothing is folded into a table that could not be read**, which would restart every average from one day; and the write carries a `lastDay` guard so two processes in a deploy's overlap cannot write a day twice.
