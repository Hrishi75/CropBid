...
//
// Fetched once per page load and shared: the cart, the checkout and the sticky
// bar all ask, and none of them should wait on a round trip the others made.
...
function loadRules(): Promise<RetailRules> {
  ...
  if (!inFlight) {
    inFlight = api.get<RetailRules>('/browse/retail-rules')
      .then(({ data }) => {
        cached = data;
        return data;
      })
