...
/**
 * WHY ONE COMPONENT FOR BOTH VIEWS?
 * Farmers and buyers see the same bid data but with different actions:
 *   - Farmer: Accept / Reject / Counter
 *   - Buyer: Update price / Withdraw
 * Using `viewAs` prop avoids duplicating the card layout.
 */
  ...
  async function handleAction(action: string) {
    ...
    try {
      if (action === 'accept') {
        ...
        if (!confirm('Withdraw this bid?')) { setLoading(''); return; }
  ...
  return (
    <Card>
      <div className="flex items-start justify-between mb-3">
              ...
              <div>
                ...
                {bid.buyer.trustScore !== undefined && (
                  <p className="text-xs text-text-secondary flex items-center gap-1">
                    <Star size={10} /> {Math.round(bid.buyer.trustScore)}/100
                  </p>
                )}
              </div>
            </div>
        ...
        {/* Status badge */}
        <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${statusStyles[bid.status] || ''}`}>
          {bid.status}
        </span>
      </div>
      ...
      {viewAs === 'buyer' && (bid.status === 'PENDING' || bid.status === 'COUNTERED') && (
        <div className="flex gap-2">
          ...
          <Button size="sm" variant="danger" onClick={() => handleAction('withdraw')} loading={loading === 'withdraw'}>
            Withdraw
          </Button>
        </div>
      )}
