...
// opaque tokens above.
export const design = {
  // The page GROUND, deliberately a step darker than `paper`. Cards used to sit
  // on surfaceAlt, about 2% away from their own fill, so they read as flat
  // patches rather than as cards and the shadow had nothing to separate.
  bg: colors.surfaceHover, //  #efece3
...
// Elevation
// -----------------------------------------------------------------------------
// Cards sit on a warm ground barely lighter than themselves, so a border alone
// leaves them flat. A shadow this soft reads as lift rather than as a drop
// shadow, and the colour is the brand ink rather than black, which on a cream
// background otherwise goes grey and dirty.
//
// `elevation` is Android's own token and is required there: iOS and web read
// shadowColor/Offset/Opacity/Radius and ignore it, Android reads only this.
export const shadow = {
  card: {
    shadowColor: '#14140f',
