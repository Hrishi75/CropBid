...
// The same map exists in client/src/utils/cropImages.ts,
// mobile/src/utils/cropImages.ts and server/prisma/cropImages.ts. This is a
// fourth copy, so keep all four in sync.
//
// NOT every crop has a photo, and that is expected rather than a gap to paper
// over: coriander has none today. Callers must handle null by drawing a real
// placeholder, because an empty grey tile on a shelf row reads as a broken
// image rather than as a missing one.
// =============================================================================
